...
export default function AddCategory() {
   ...
   const token = localStorage.getItem("token")
      ...
      async function getData(file , values){
         ...
         axios.post("https://free-palestine-back-end.onrender.com/api/v1/categories" ,formData,{headers})
         .then((response)=>{
         ...
         <Formik
               ...
               {({ isSubmitting, setFieldValue }) => (
                  ...
                  <Form className='w-75 m-auto'>
                     <h1 className='main-header'>Add New Category</h1>
                     {  error? <div className='alert alert-danger mt-5' >{error}</div> : ""}


                     <div  className='mt-2'> 
                           <label htmlFor="name" className="form-label">Add Category Name</label>
